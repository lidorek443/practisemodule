...
<!-- Copyright 1998-2022 by Northwoods Software Corporation. -->    <title> GoJS with ES6 Modules -- Northwoods Software </title>
  ...
  <a href="typings.html">Typings</a>
    ...
    <p>
      The GoJS installation contains the GoJS library as an ES6 module,
      extensions as ES6 modules, and examples of using GoJS with ES6 modules.
    </p>
    ...
    <p>
      The GoJS library file "go.js" does not use the <code>export</code> keyword,
      for compatibility with all JavaScript environments.
      So we have had to provide different libraries that are ES6 modules.
      In the <code>release</code> folder, they are the <code>go-module.js</code> and <code>go-debug-module.js</code> files.
      In addition, there is an ES6 module-specific TypeScript definition file: <code>go-module.d.ts</code>.
    </p>
    <p>
      In the <code>extensionsJSM</code> directory, all of the samples use ES6 modules.
      Each of the JavaScript or TypeScript files imports the GoJS library this way:
    </p>
<pre class="lang-ts">  <code>import * as go from '../release/go-module.js';</code></pre>

    <p>In order to load GoJS and other ES6 modules, the HTML page also must use the <code>type="module"</code> script tag:</p>
<pre class="lang-html"><code>  &lt;script type="module"&gt;
  ...
  &lt;/script&gt;
    ...
    <p>
      ...
      so you will need to open an HTML page using modules from a web server to see the results on localhost:.
    </p>

    <h3 id="UsingExtensionModules">Using Extension Modules</h3>
    <p>
  ...
  &lt;/script&gt;</code></pre>
    ...
    <p>
      Note that all those modules the depend on GoJS must import exactly the same library file,
      in this case "../release/go-module.js".
      Any inconsistencies will cause the GoJS library to be loaded multiple times,
      which can cause subtle errors and thus results in the library signaling error.
    </p>
    <p>
      We recommend that you copy the extension file(s) that you want to use into your own project so that you can be sure that
      all of the <code>import</code> statements import the same GoJS library.
      The extensions are all in TypeScript and were all compiled with this <code>tsconfg.json</code> file:
    </p>
  ...
  <pre class="lang-html"><code>  &lt;script src="https://cdnjs.cloudflare.com/ajax/libs/require.js/2.3.6/require.min.js"&gt;&lt;/script&gt;
  &lt;script id="code"&gt;
  function init() {
